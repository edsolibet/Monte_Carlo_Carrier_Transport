...
#num_carr = init.num_carr
#Q = init.Q # charge of superparticles for charge neutrality

# calculate energy in eV
# to show histogram, extract kx, ky, kz from coords using ki = coords[;,i]
def calc_energy(kx, ky, kz, mass):
    E = (kx**2 + ky**2 + kz**2)*(hbar)**2/(2*mass*Q)
    if m.isnan(E):
        print ('kx', kx)
        print ('ky', ky)
        print ('kz', kz)
...
# Density of states
def density_of_states(E, m):
    return 2 * (2*m)**(1.5) * np.sqrt(E) / (4*np.pi**2 * hbar_eVs**3)
...
def carrier_drift(coord, elec_field, dt2, mass):
    ...
    x1 = x + hbar*kx*dt2/mass
    y1 = y + hbar*ky*dt2/mass
    #z1 = z + (calc_energy(kx1, ky1, kz1) - calc_energy(kx, ky, kz))/(-elec_field*1E2)
    z1 = z + (hbar*kz - 0.5*Q*elec_field*1E2*dt2)*(dt2/mass)
...
def inv_debye_length(N):
    return np.sqrt(Q*N*1E2**3 / (GaAs.eps_0*kT))

def plasmon_freq(N, mass):
    return np.sqrt(Q**2 * N*1E2**3/ (GaAs.eps_0*mass))
